  ...
  <ul>
   ...
   <li>
    <table class="price_table">
     <caption>
      <strong>${pageInfo.time}</strong>
      <strong>询价单号：${pageInfo.inquiryid}</strong>
     </caption>
     <tr>
      <th>序号</th>
      <th>产品配置</th>
     ...
     <tr>
      <td class="center">${pageInfo.inquiryid}</td>
      <td>${pageInfo.configure}</td>
      <td>${pageInfo.material}</td>
      <td>${pageInfo.type}</td>
      <td class="center">${pageInfo.thickness}</td>
      <td>${pageInfo.color}</td>
      <td>${pageInfo.working}</td>
      <td class="center">${pageInfo.num}</td>
      <td class="center"><a href="/order/deleteInquiry.do?inquiryid=${pageInfo.inquiryid}"><input type="button" value="删除询价单"/></a></td>
     </tr>
     <tr>
      <td class="center"></td>
      <td>${pageInfo.configure}</td>
      ...
      <td>${pageInfo.working}</td>
      <td class="center">${pageInfo.num}</td>
      <td class="center"><input type="button" value="报价详情" onClick="javascript:location.href='${pageContext.request.contextPath}/order/findOneInquiry.do?inquiryid=${pageInfo.inquiryid}'"/></td>
     </tr>
    </table>
   </li>
   </c:forEach>
   ...
   <!--分页-->
   <div class="paging">
    <a href="${pageContext.request.contextPath}/order/findInquiry.do?currentpage=1">第一页</a>
    <c:forEach var="i" begin="1" end="${pageInfo.pages}">
     <a href="${pageContext.request.contextPath}/order/findInquiry.do?currentpage=${i}">${i}</a>
    </c:forEach>
    <a href="${pageContext.request.contextPath}/order/findInquiry.do?currentpage=${pageInfo.pages}">最后一页</a>
   </div>
 </div>
</section>
<!--footer-->
