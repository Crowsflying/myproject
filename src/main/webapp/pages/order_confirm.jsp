...
<section class="wrap" style="margin-top:20px;overflow:hidden;">
 <table class="order_table">
  <caption>
   <strong>订单商品</strong>
   <a href="${pageContext.request.contextPath}/clothes/findCart.do">返回购物车修改</a>
  </caption>
  <c:forEach var="settlement" items="${settlement}">
  <tr>
   <td class="center"><a href="#"><img src="http://q28msmys0.bkt.clouddn.com/${settlement.image}" style="width:50px;height:50px;"/></a></td>
   <td><a href="#">${settlement.goodsname}</a></td>
   <td>
    <p>颜色：黑色</p>
    <p>规格：M码</p>
   </td>
   <td class="center"><span class="rmb_icon">${settlement.price}</span></td>
   <td class="center"><span>${settlement.nums}</span></td>
   <td class="center"><strong class="rmb_icon">${settlement.price*settlement.nums}</strong></td>
  </tr>
  </c:forEach>
 </table>
 <!--支付与配送-->
  ...
  <li>
   <dl>
    ...
    <dd>
     <label>
      <input type="checkbox"/>
      使用余额￥<input type="text" value="<%=user.getBalance()%>" class="textbox"/>
      <mark>当前账户余额<strong class="rmb_icon"><%=user.getBalance()%></strong></mark>
     </label>
    </dd>
   </dl>
  </li>
 ...
 <div class="order_btm_btn">
  <a href="${pageContext.request.contextPath}/order/submitOrder.do?buybuybuy=${buybuybuy}" class="link_btn_02 add_btn" />共计金额<strong class="rmb_icon">${buybuybuy}</strong>提交订单</a>
 </div>
</section>
<!--footer-->
<footer>
