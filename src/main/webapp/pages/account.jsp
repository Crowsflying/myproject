...
$(document).ready(function(){
  ...
  $("#tixian").click(function(){
    ...
    $(".group_btn").on("click",function(){
        if ($(".group_btn").val()=="立即充值") {
            var accounttype="充值";
            var transaction=$(".textbox").val();
            if (transaction==null||transaction<=0){
                alert("充值失败");
                return
            }
            $.ajax({
                url:"${pageContext.request.contextPath}/onself/updatamoney.do",
                type:"post",
                dataType:"json",
                async:true,
                data:{
                ...
                success:function () {
                        alert("充值成功，你账户的余额为∞");
                        window.location.href="${pageContext.request.contextPath}/onself/findaccount.do";
                }
            })
...
<section class="wrap user_center_wrap">
 ...
 <div class="user_rt_cont">
  ...
  <dl class="user_account">
   <dt>账户余额：</dt>
   <dd>
    <strong class="rmb_icon"><%=user.getBalance()%></strong>
  ...
  <table class="order_table">
   ...
   <c:forEach var="account" items="${pageInfo.list}">
   <tr>
    <td class="center">${account.accounttype}</td>
    <td class="center"><strong class="rmb_icon">${account.transaction}</strong></td>
    <td class="center"><span class="rmb_icon">${account.thatbalance}</span></td>
    <td class="center"><time>${account.accounttime}</time></td>
    <td class="center">${account.accounttype}完成！</td>
   </tr>
   </c:forEach>
  </table>
   <!--分页-->
   <div class="paging">
    <a href="${pageContext.request.contextPath}/onself/findaccount.do?currentpage=1">第一页</a>
    <c:forEach var="i" begin="1" end="${pageInfo.pages}">
     <a href="${pageContext.request.contextPath}/onself/findaccount.do?currentpage=${i}">${i}</a>
    </c:forEach>
    <a href="${pageContext.request.contextPath}/onself/findaccount.do?currentpage=${pageInfo.pages}">最后一页</a>
   </div>
 </div>
</section>
<!--footer-->
